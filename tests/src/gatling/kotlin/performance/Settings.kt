...
class Settings {
    ...
    val usersPerSec =
        requireNotNull(System.getProperty(USER_PER_SEC_PROPERTY)) {
            "Property $USER_PER_SEC_PROPERTY is null"
        }.toDouble()

    val usersSize =
        requireNotNull(System.getProperty(MENU_SIZE_PROPERTY)) {
            "Property $MENU_SIZE_PROPERTY is null"
        }.toInt()

    val duration =
        Duration.parse(
            requireNotNull(System.getProperty(DURATION_PROPERTY)) {
